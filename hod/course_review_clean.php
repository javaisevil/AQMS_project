...
<div style="display:flex; gap:20px; align-items:flex-start; flex-wrap:wrap;">
    <div style="flex:1; min-width:0;">
        <div class="card">
            ...
            <table style="margin-bottom:0;">
                <tr><td style="width:180px; color:var(--text-muted);">Due Date</td><td><?php echo htmlspecialchars($course['due_date'] ?? 'Not set'); ?></td></tr>
                <tr><td style="color:var(--text-muted);">Submitted At</td><td><?php echo htmlspecialchars($course['submitted_at'] ?? 'Not submitted'); ?></td></tr>
                <tr><td style="color:var(--text-muted);">Deadline Status</td><td><?php echo htmlspecialchars(str_replace('_', ' ', $course['deadline_status'] ?? 'not_due')); ?></td></tr>
                <tr><td style="color:var(--text-muted);">Level</td><td><?php echo htmlspecialchars($course['course_level']); ?></td></tr>
            ...
            <table>
                ...
                <?php foreach ($clos as $clo): ?>
                    <tr>
                        ...
                        <td><?php echo $clo['plo_codes'] ? htmlspecialchars($clo['plo_codes']) : 'Not mapped'; ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div style="width:320px; min-width:280px;">
        ...
        <div class="card">
            ...
            <form method="POST">
                <div class="form-group"><label>Comment / Feedback</label><textarea id="comment" name="comment" rows="5" oninput="checkComment()"></textarea></div>
                <button type="submit" name="action" value="approve" class="btn btn-success btn-full">Approve and Forward to QA</button>
                <button type="submit" id="rejectBtn" name="action" value="reject" class="btn btn-danger btn-full" disabled>Return to Faculty</button>
            </form>
        </div>
        <?php else: ?>
        <div class="card"><p class="text-muted text-center">No HoD action required for this status.</p></div>
        <?php endif; ?>
        ...
        <div class="card">
            ...
            <ul class="timeline">
                ...
                <li>
                    <div class="timeline-date"><?php echo date('M d, Y H:i', strtotime($log['created_at'])); ?> <span class="role-badge role-<?php echo $log['role']; ?>"><?php echo strtoupper($log['role']); ?></span></div>
                    <div class="timeline-text"><?php echo htmlspecialchars($log['full_name']); ?>: <?php echo htmlspecialchars(str_replace('_', ' ', $log['from_status'] ?: 'New')); ?> to <?php echo htmlspecialchars(str_replace('_', ' ', $log['to_status'])); ?></div>
                    <?php if ($log['comment']): ?><div class="timeline-comment">"<?php echo htmlspecialchars($log['comment']); ?>"</div><?php endif; ?>
                </li>
                <?php endforeach; ?>
            </ul>
            <?php endif; ?>
...
function checkComment() {
    ...
    if (!commentBox || !rejectBtn) return;
    rejectBtn.disabled = commentBox.value.trim().length === 0;
}
